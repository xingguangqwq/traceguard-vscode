...
const { buildSemanticSymbolIndex, semanticCandidates } = require("../analysis/semantic-symbol-index");

const javaRelationIndexes = new WeakMap();
...
function buildFunctionIndex(functions) {
  ...
  Object.defineProperty(index, "semanticSymbols", {
    value: buildSemanticSymbolIndex(functions),
    enumerable: false,
  });
  Object.defineProperty(index, "resolutionCache", {
    value: new WeakMap(),
    enumerable: false,
  });
  return index;
...
function resolveCandidates(index, event, caller) {
  const cached = cachedResolution(index, event, caller);
  if (cached) return cached;
  const reference = findEventReference(caller.references, event);
  const indexed = semanticCandidates(index.semanticSymbols, event, caller);
  const indexedIds = new Set(indexed.map(candidate => candidate.id));
  const namedCandidates = indexed.length && ["csharp", "go"].includes(caller.language)
    ? indexed
    : (index.get(event.callee.toLowerCase()) || []);
  // Large servlet projects commonly contain thousands of doGet/doPost methods.
  // An unqualified call to the sole same-file declaration is lexically local;
  // scoring every project-wide homonym first makes dependency construction
  // quadratic without adding any resolution evidence. Imports and explicit
  // non-self receivers deliberately keep the full semantic-resolution path.
  const candidates = uniqueLexicalCandidate(namedCandidates, event, caller, reference) || namedCandidates;
  const scored = candidates
    ...
    .map(candidate => scoreCandidate(candidate, caller, event, reference, indexedIds.has(candidate.id)))
  ...
  if (!scored.length) return storeResolution(index, event, caller, []);
  ...
  // A lone same-named function is still not a symbol proof. Without receiver,
  // import, explicit target, same-file, or semantic-index evidence the edge is
  // useful for navigation only and must remain heuristic.
  return storeResolution(index, event, caller,
    selected.map(item => ({ ...item, quality: ambiguous || !item.strong ? "review" : "high" })));
}

function cachedResolution(index, event, caller) {
  const byCaller = index.resolutionCache?.get(event);
  return byCaller?.get(caller.id);
}

function storeResolution(index, event, caller, resolutions) {
  if (!index.resolutionCache || !event || typeof event !== "object") return resolutions;
  let byCaller = index.resolutionCache.get(event);
  if (!byCaller) {
    byCaller = new Map();
    index.resolutionCache.set(event, byCaller);
  }
  byCaller.set(caller.id, resolutions);
  return resolutions;
}

function uniqueLexicalCandidate(candidates, event, caller, reference) {
  if (event.targetFunctionId || reference?.kind === "import") return undefined;
  const receiver = canonicalName(event.receiver);
  if (receiver && !["this", "self", "cls"].includes(receiver)) return undefined;
  const callerPath = normalizePath(caller.absolutePath);
  const local = candidates.filter(candidate => normalizePath(candidate.absolutePath) === callerPath);
  return local.length === 1 ? local : undefined;
}

function scoreCandidate(candidate, caller, event, reference, semanticIndexed = false) {
  ...
  const strong = Boolean(semanticIndexed || exactTarget || sameFile || typeResolution.match || referenceMatch === "exact" || receiverTypeMatch || receiverMatch);
  const semanticReason = semanticIndexed && ["csharp", "go"].includes(caller.language);
  const reason = exactTarget ? "explicit closure target" : semanticReason ? `semantic symbol ${event.receiverType}.${event.callee}/${event.arguments?.length || 0}` : typeResolution.match ? `resolved type ${typeResolution.target}` : receiverContractMatch ? `receiver contract ${event.receiverType}` : receiverScopeMatch ? `receiver type ${event.receiverType}` : semanticIndexed ? `semantic symbol ${event.receiverType}.${event.callee}/${event.arguments?.length || 0}` : sameFile ? "same file" : referenceMatch !== "none" ? `import ${reference.target}` : receiverMatch ? `receiver ${event.receiver}` : sameDirectory ? "nearest file" : "function name";
  return {
    fn: candidate,
    score: (semanticIndexed ? 48 : 0) + (exactTarget ? 40 : 0) + (typeResolution.match ? 36 : 0) + (receiverTypeMatch ? 24 : 0) + (referenceMatch === "exact" ? 20 : referenceMatch === "name" ? 6 : 0) + (sameFile ? 16 : 0) + (receiverMatch ? 12 : 0) +
      (arityMatch ? 8 : -8) + argumentTypeScore + (sameDirectory ? 3 : 0) + (candidate.language === caller.language ? 2 : 0),
    strong,
    precise: Boolean(semanticIndexed || exactTarget || typeResolution.match || receiverTypeMatch || referenceMatch === "exact" || receiverMatch),
...
function findEventReference(references = [], event) {
  const receiver = canonicalName(event.receiver);
  // For unqualified Java calls the lightweight reference table can contain
  // `<callee> -> <return type>` as a type reference.  It is not receiver
  // evidence and must not steer the call to the imported return class.
  // Qualified calls still resolve their receiver declaration/type first.
  const direct = receiver
    ? references.find(reference => canonicalName(reference.local) === receiver)
    : references.find(reference => canonicalName(reference.local) === canonicalName(event.callee) && reference.kind !== "type");
...
function javaTypeAssignable(candidateType, target, candidate) {
  ...
  const relations = javaRelationIndex(candidate.typeRelations || []);
  const candidateRelations = (candidate.implementedTypes || []).map(type => resolveJavaType(candidate, type)).filter(Boolean);
  ...
  while (pending.length) {
    ...
    visited.add(current);
    pending.push(...(relations.get(current) || (current === candidateType ? candidateRelations : [])));
  }
  return false;
}

function javaRelationIndex(typeRelations) {
  if (!typeRelations || typeof typeRelations !== "object") return new Map();
  let index = javaRelationIndexes.get(typeRelations);
  if (index) return index;
  index = new Map();
  for (const relation of typeRelations) {
    const type = normalizeJavaType(relation.type);
    if (!type) continue;
    index.set(type, (relation.extends || []).map(normalizeJavaType).filter(Boolean));
  }
  javaRelationIndexes.set(typeRelations, index);
  return index;
}
